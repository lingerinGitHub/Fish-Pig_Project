...
const formData = reactive<order>({
    ...
    bead: null, // 珠
    totalWeight: null,  // 总重
    grainPerCatty: null, // 每公斤粒数
    totalNumberOfGrains: null, // 总粒数
    unitPrice: null,// 单价
    amount: null, // 金额
...
const title = computed(() => {
    return `向 ${props.corporation.corporationName} 添加订单信息`
})
    ...
    try {
        ...
        await useOrderStore().addOrder(formData)
            .then(() => {
                ...
                mitt.emit('ElNotification', { type: 'success', title: "成功", message: '订单添加成功' })
            })
            .catch((err: any) => {
...
watch(
    ...
    (newVal) => {
        ...
        formData.type = props.corporation.type
    },
